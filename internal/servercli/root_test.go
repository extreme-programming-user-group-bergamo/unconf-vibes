...
func TestNewRootCmd(t *testing.T) {
	...
	assert.Equal(t, "unconf-server", cmd.Use)
	assert.Contains(t, cmd.Short, "UNCONF Server CLI")
}
...
func TestRootCmdHelpFlag(t *testing.T) {
	...
	assert.Contains(t, helpOutput, "UNCONF Server CLI provides backend and operational commands")
	assert.Contains(t, helpOutput, "serve")
	assert.Contains(t, helpOutput, "db")
}
...
func TestRootCmdVersionFlag(t *testing.T) {
	...
	assert.Contains(t, versionOutput, "unconf-server")
}

func TestRootCmdIncludesServeCommand(t *testing.T) {
	cmd := NewRootCmd()
