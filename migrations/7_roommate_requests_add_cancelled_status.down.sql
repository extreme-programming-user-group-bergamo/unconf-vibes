CREATE TABLE roommate_requests_old (
...
INSERT INTO roommate_requests_old (id, requester_id, target_id, room_id, status, created_at)
SELECT
    id,
    requester_id,
    target_id,
    room_id,
    CASE WHEN status = 'cancelled' THEN 'declined' ELSE status END,
    created_at
...
ALTER TABLE roommate_requests_old RENAME TO roommate_requests;
